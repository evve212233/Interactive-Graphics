...
point3 track[] = { point3(-4, 1, 4), point3(-1,1,-4), point3(3,1,5) };
...
float calcDistance(float x, float y, float z) {
...
point3 calculateDirection(point3 from, point3 to) {
	...
	float d = calcDistance(v.x, v.y, v.z);
	v.x = v.x / d;
	v.y = v.y / d;
	v.z = v.z / d;
...
void loadSphereFile()
{
	...
	file >> col;
	std::cout << col;
	sphere_points = new point3[col * 3];
	sphere_shadow_points = new point3[col * 3];
	sphere_color = new point3[col * 3];
	sphere_shadow_colors = new point3[col * 3];
	...
	for (int i = 0; i < col; i++) {
		...
		for (int j = 0; j < points; j++) {
			...
			sphere_shadow_points[count] = tmp2;
			sphere_shade_d[count] = tmp_shade;
			count++;
			//	cout << sphere_points[count - 1] << endl;
		}
	}
...
void colorsphere()
{
	for (int i = 0; i < col * 3; i++) {
		...
		sphere_shadow_colors[i] = color3(0.25, 0.25, 0.25);
	}
...
void init()
{
	...
	glBufferData(GL_ARRAY_BUFFER, 3 * col * sizeof(point3) + 3 * col * sizeof(color3),
		NULL, GL_STATIC_DRAW);
	glBufferSubData(GL_ARRAY_BUFFER, 0, 3 * col * sizeof(point3), sphere_shadow_points);
	glBufferSubData(GL_ARRAY_BUFFER, 3 * col * sizeof(point3), 3 * col * sizeof(color3),
...
int main(int argc, char **argv)
{
	...
	int shadeMenu = glutCreateMenu(shade_menu);
	glutAddMenuEntry("flat shading", 1);
	glutAddMenuEntry("smooth shading", 2);

	int lightMenu = glutCreateMenu(lighting_menu);
	glutAddMenuEntry("No", 1);
	glutAddMenuEntry("Yes", 2);
	...
	glutAddSubMenu("Lighting", lightSource);
